...
function Step({ label, state, detail }: { label: string; state: StepState; detail?: string }) {
  ...
  return (
    <li className="flex items-center gap-3 text-sm">
      <span className={`grid h-6 w-6 shrink-0 place-items-center rounded-full text-xs font-bold ${dot}`}>{mark}</span>
      <span>
        {label}
...
export function ConnectionPanel({ connection, onConnect, onSync, onDisconnect, problem }: ConnectionPanelProps) {
  const property = connection?.lastRuns.find((run) => run.entity === "PROPERTY");
  const reservation = connection?.lastRuns.find((run) => run.entity === "RESERVATION");
  const propertiesDone = !!property && property.status !== "RUNNING";
  const allDone = propertiesDone && !!reservation && reservation.status !== "RUNNING";
    ...
    <section className="rounded-2xl border border-zinc-200 bg-white p-5 shadow-sm dark:border-zinc-800 dark:bg-zinc-900">
      <div className="flex flex-wrap items-center justify-between gap-4">
          ...
          <div>
            ...
            {connection ? (
              <p className="flex flex-wrap items-center gap-2 text-sm text-zinc-500">
                ...
                {connection.lastSyncedAt && <span>· last synced {formatDateTime(connection.lastSyncedAt)}</span>}
              </p>
            ) : (
              <p className="text-sm text-zinc-500">Bring your properties and reservations into BoxBuy.</p>
            )}
          </div>
        </div>
        ...
        <div className="flex gap-2">
          {connection ? (
            <>
              <button onClick={onSync} className="rounded-lg border border-zinc-300 px-4 py-2 text-sm font-semibold dark:border-zinc-700">
                Sync now
              </button>
              <button onClick={onDisconnect} className="rounded-lg border border-zinc-300 px-4 py-2 text-sm font-semibold text-red-700 dark:border-zinc-700 dark:text-red-400">
                Disconnect
              </button>
            </>
          ) : (
            <button onClick={onConnect} className="rounded-lg bg-blue-600 px-5 py-2 text-sm font-semibold text-white hover:bg-blue-700">
              Sync with GPC
            </button>
          )}
        </div>
      ...
      {connection && (
        <>
          <ol className="mt-4 space-y-2">
            ...
            <Step label="Import properties" state={property ? stateOf(property) : "run"} detail={detailOf(property)} />
            <Step
              label="Import reservations"
              state={propertiesDone ? (reservation ? stateOf(reservation) : "run") : "wait"}
          ...
          {allDone && connection.status === "CONNECTED" && (
            <p className="mt-4 rounded-lg bg-emerald-100 p-3 text-sm text-emerald-800 dark:bg-emerald-950 dark:text-emerald-300">
              All set. {connection.imported.properties} properties and {connection.imported.reservations} reservations are in GPC.
            </p>
          )}
          {(connection.status === "FAILED" || connection.status === "TIMEOUT") && (
